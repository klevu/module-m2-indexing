...
{
    public const KLEVU_INDEXING_ATTRIBUTES_ACTION_UPDATE_AFTER = 'klevu_indexing_attributes_action_update_after';
    ...
    public function execute(
        ...
        $response = $this->updateAttributeViaSdk(
            accountCredentials: $accountCredentials,
            attribute: $attribute,
        );
        $this->handleResponse(
     ...
     * @throws InvalidAccountCredentialsException
     */
    private function updateAttributeViaSdk(
        AccountCredentials $accountCredentials,
        SdkAttributeInterface $attribute,
    ): SyncResultInterface {
        try {
            $response = $this->attributesService->put(
                accountCredentials: $accountCredentials,
                attribute: $attribute,
            );
    ...
    private function dispatchEvent(
        ...
        $this->eventManager->dispatch(
            static::KLEVU_INDEXING_ATTRIBUTES_ACTION_UPDATE_AFTER,
            [
    ...
    private function logFailure(
        ...
        $this->logger->error(
            ...
            context: [
                'method' => __METHOD__,
                'message' => sprintf(
                    '%s: Attribute %s failed to sync with Klevu: %s',
                    $response->getCode(),
